                            ...
                            int *damageDealt, int *defender_hp_remaining, int *seq_num)
{
    ...
    if (detect_game_over(buffer, winner, loser, &endseq)) {
        printf("\nGAME OVER detected in RESOLUTION!\nWinner: %s | Loser: %s\n", winner, loser);
        exit(0);
    }

    sscanf(strstr(buffer, "attacker:"), "attacker: %49s", attacker);
    sscanf(strstr(buffer, "move_used:"), "move_used: %49s", move_used);
    sscanf(strstr(buffer, "damage_dealt:"), "damage_dealt: %d", damageDealt);
    sscanf(strstr(buffer, "defender_hp_remaining:"), "defender_hp_remaining: %d", defender_hp_remaining);
